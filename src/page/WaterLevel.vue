<template>
  <div style="height: 92vh; width:100%">
    ...
    <Modal class="iModal" v-model="baojingModal" title="报警主机" :scrollable="true" :footer-hide="false">
      ...
      <div style="margin: 10px;overflow: hidden">
        <div style="float: right;">
          <Page :total="total_baojingData" size="small" :pageSize="5" show-total @on-change="changePage2"></Page>
        </div>
      </div>
      <div slot="footer"></div>
    </Modal>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        deviceColumns:[
          ...
          {slot: 'action', title: '查看', align: 'center'}
        ],
        deviceListData: [],
        deviceType: [],//大类别
        smallDeviceType: [],//小类别
    ...
    methods: {
      ...
      addingMulch() {
        ...
        loadBMap('isZkHarwgZspmmnDOBpTGgGDpoMKRBAx').then(() => {
          ...
          /////////////////////////////////////////////默认打开第一个
          let point = new BMap.Point(this.deviceStatus[0].lon, this.deviceStatus[0].lat);
            // 创建标注对象并添加到地图
            this.markerTemp[0] = new BMap.Marker(point);
            this.markerTemp1[0] = this.deviceStatus[0];
            let _that = this;
                    ...
                    //默认展开抽屉，水压设备信息
                      ...
                      _that.$ajax.get('/zigbee/selectDeviceOuterDeviceType').then((res) => {
                        if (res.data.code == 200) {
                          ...
                          _that.deviceName = res.data.data[1].id;
    ...
    historyData(data) {
      //查看历史数据
      this.changeDevice('618e9b030000_L1.127', '水位监控');
      this.zhixianModal= true;
    },
