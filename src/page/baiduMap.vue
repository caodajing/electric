<template>
  <div style="height: 92vh; width:100%">
    <Row type="flex" align="middle" style="width: 100%; height: 8vh">
      ...
      <Col span="5"><Input suffix="ios-search" v-model="searchContent" @on-change="inputChange" placeholder="输入单位名称开始查找" /></Col>
      <Col span="1">区域：</Col>
      <Col span="4">
    ...
    <Modal class="iModal" v-model="newModal" :title="newModalTitle" :scrollable="true" :mask="false" :footer-hide="false">
                  ...
                  _that.myMap.centerAndZoom(centerPointer, 19);
                    ...
                    if (res.data.code == 200) {
                      ...
                      //获取点击地图中的该单位的设备详情
                        ...
                        var content = `<h2>${unitList[0].unit_name}</h2>
                                  <span><i style="font-size:11px" class="iconfont icon-dingwei"></i>${unitList[0].address}</span>
                                  <ul style="width:90%; font-size:14px">
                                    <li style="width:100%;border-bottom:1px solid #f3f3f3;line-height:30px">
                                      <span style="display:inline-block; width:50%">报警主机</span>
                                      <span style="display:inline-block; width:50%;cursor:pointer;float:right;text-align:right" class="temp-li1" @click="click1()">${_that.totalCount}  <i class="iconfont icon-tixing" style="color:red"></i> <i class="iconfont icon-icon-test" style="color:#c0b9b9"></i></span>
                                    </li>
                                    <li style="width:100%;border-bottom:1px solid #f3f3f3;line-height:30px;clear:both">
                                      <span style="display:inline-block; width:50%">水压</span>
                                      <span style="display:inline-block; width:50%;cursor:pointer;float:right;text-align:right" class="temp-li2"><i class="iconfont icon-icon-test" style="color:#c0b9b9"></i></span>
                                    </li>
                                    <li style="width:100%;border-bottom:1px solid #f3f3f3;line-height:30px;clear:both">
                                      <span style="display:inline-block; width:50%">水位</span>
                                      <span style="display:inline-block; width:50%;cursor:pointer;float:right;text-align:right" class="temp-li3"><i class="iconfont icon-icon-test" style="color:#c0b9b9"></i></span>
                                    </li>
                                    <li style="width:100%;border-bottom:1px solid #f3f3f3;line-height:30px;clear:both">
                                      <span style="display:inline-block; width:50%">电气火灾</span>
                                      <span style="display:inline-block; width:50%;cursor:pointer;float:right;text-align:right" class="temp-li4"><i class="iconfont icon-icon-test" style="color:#c0b9b9"></i></span>
                                    </li>
                                  </ul>`;
                        var infoWindow = new BMap.InfoWindow(content, opts);  // 创建信息窗口对象 
                        _that.myMap.openInfoWindow(infoWindow,point); //开启信息窗口
                      ...
                      setTimeout(function(){
                        document.getElementsByClassName("temp-li1")[0].addEventListener('click',function(){
                          _that.$router.push("/alarmHost");
                        });
                        document.getElementsByClassName("temp-li2")[0].addEventListener('click',function(){
                          //水压
                          _that.$router.push("/waterGage");
                        });
                        document.getElementsByClassName("temp-li3")[0].addEventListener('click',function(){
                          //水位
                          _that.$router.push("/waterLevel");
                        });
                        document.getElementsByClassName("temp-li4")[0].addEventListener('click',function(){
                          //电气火灾
                          _that.$router.push("/electricalFire");
                        });
                      },500)
                    })
                }
      ...
      this.$ajax.get('/zigbee/zigbeeWebMapDeviceListInUnit',
        {
          params: {
            ...
            type: 1,
            deviceTypeId: this.smallId?this.smallId: '',
